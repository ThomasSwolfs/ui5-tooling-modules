...
module.exports = async function ({ workspace, dependencies, taskUtil, options }) {
	...
	function rewriteDep(dep, useDottedNamespace) {
		if (config.addToNamespace) {
			...
			d = `${options.projectNamespace}/blabla/thirdparty/${d}`;
		...
		Array.from(uniqueDeps).map(async (dep) => {
			...
			if (bundle) {
				...
				const bundleResource = resourceFactory.createResource({
					path: `/npmmodules/${rewriteDep(dep)}.js`,
					string: bundle,
				});
		...
		Array.from(uniqueResources).map(async (resource) => {
			...
			if (content) {
				...
				const newResource = resourceFactory.createResource({
					path: `/npmmodules/${rewriteDep(resource)}`,
					string: content,
				});
	...
	if (!config?.addToNamespace && config?.prependPathMappings) {
		const resComponent = await workspace.byPath(`/npmmodules/${options.projectNamespace}/Component.js`);
		if (resComponent) {
			let pathMappings = "";
			Array.from(bundledResources).map(async (resource) => {
				pathMappings += `"${resource}": sap.ui.require.toUrl("${options.projectNamespace}/npmmodules/${resource}"),`;
			});
